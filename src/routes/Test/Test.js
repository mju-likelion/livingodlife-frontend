  ...
  function setChallenge(point) {
    ...
    challengeBtn.onclick = function () {
      axios
        ...
        .then(() => {
          navigate(`/challenge/${challengeList[point].link}`);
        });
      axios.post("/client/test","",{
        headers: {
          Authorization: localStorage.getItem("login-token"),
        },
      }).then(
  ...
  return (
    <>
      <div id="startPage" ref={startRef}>
        ...
        <p className="GmarketM" id="testContent">
          당신의 ‘GOD생’을 테스트 해보세요!
        </p>
        <button className="GmarketS" id="startBtn" onClick={start}>
          테스트 시작
          <div id="circle">
      ...
      <div id="qnaPage" ref={qnaRef}>
        <h2 id="sidePage" className="jalan">
          TEST PAGE
        </h2>
        <button className="sideBtn GmarketS" onClick={restart}>
          돌아가기
        </button>
        <div className="count GmarketS" ref={count}>
          1/25
        </div>
      ...
      <div id="resultPage" ref={resultRef}>
        ...
        <div className="resultName jalan"></div>
        <div className="resultNameIntro GmarketM"></div>
        <div className="resultCont1 GmarketS"></div>
        <h2 className="GmarketS subTitle">나의 특징은?</h2>
        <div className="resultCont2 GmarketS"></div>
